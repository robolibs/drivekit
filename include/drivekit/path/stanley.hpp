        ...
        class StanleyFollower : public Controller {
          public:
            ...
            inline VelocityCommand compute_control(const RobotState &current_state, const Goal &goal,
                ...
                double k_cte = 1.0;
                double velocity = std::max(std::abs(cmd.linear_velocity), 0.1);
            ...
            inline std::string get_type() const override { return "stanley_follower"; }

          private:
            inline VelocityCommand compute_goal_control(const RobotState &current_state, const Goal &goal,
