        ...
        class MCAFollower : public MPPIFollower {
          public:
            ...
            struct DynamicObstacle {
                ...
                inline void set_unimodal_prediction(const std::vector<double> &mean_x,
                                                    const std::vector<double> &mean_y, const std::vector<double> &std_x,
                                                    const std::vector<double> &std_y) {
                    modes.clear();
                    GaussianMode mode;
                    mode.weight = 1.0;
                    mode.mean_x = mean_x;
                ...
                inline void add_mode(double weight, const std::vector<double> &mean_x,
                                     const std::vector<double> &mean_y, const std::vector<double> &std_x,
                                     const std::vector<double> &std_y) {
                    GaussianMode mode;
                    mode.weight = weight;
            ...
            struct MCAConfig {
                ...
                double weight_soft_risk = 50.0;
                double weight_hard_risk = 1e6;

                double risk_threshold = 0.05;

                double min_velocity_scale = 0.1;
                double risk_slowdown_gain = 5.0;

                double robot_radius_margin = 0.5;
            ...
            inline MCAFollower() : MCAFollower(MCAConfig{}) {}

            inline explicit MCAFollower(const MCAConfig &mca_config)
                : MPPIFollower(), mca_rng_(std::random_device{}()) {
            ...
            inline VelocityCommand compute_control(const RobotState &current_state, const Goal &goal,
                ...
                if (robot_radius < 0.1) robot_radius = 0.5;
                ...
                for (size_t i = 0; i < N; ++i) {
                    ...
                    if (is_diff_drive) {
                        ...
                        mean_steering[i] = std::clamp(mean_steering[i], -constraints.max_steering_angle,
                                                      constraints.max_steering_angle);
                    }
            ...
            inline std::string get_type() const override { return "mca_follower"; }

            inline void set_mca_config(const MCAConfig &config) {
                mca_config_ = config;
            ...
            inline MCAConfig get_mca_config() const { return mca_config_; }

            inline void set_dynamic_obstacles(const std::vector<DynamicObstacle> &obstacles) {
                dynamic_obstacles_ = obstacles;
            }

            inline const std::vector<DynamicObstacle> &get_dynamic_obstacles() const { return dynamic_obstacles_; }

            inline const std::vector<std::vector<double>> &get_sample_collision_probs() const {
          ...
          protected:
            ...
            compute_collision_probabilities(const std::vector<std::vector<Point>> &trajectories, size_t timestep,
                                            double robot_radius) {
                ...
                for (size_t k = 0; k < K; ++k) {
                    ...
                    for (size_t j = 0; j < Nmc; ++j) {
                        double dx = robot_pos.x - mc_x[j];
                        double dy = robot_pos.y - mc_y[j];
            ...
            inline double evaluate_obstacle_pdf(const DynamicObstacle &obstacle, size_t timestep, double x,
                                                double y) const {
