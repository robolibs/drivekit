...
private:
    ...
    PathPoint find_nearest_path_point(const Pose& current_pose) {
        ...
        if (nearest_idx < path_.waypoints.size() - 1) {
            ...
            if (path_length > 1e-6) {
                // Normal vector to path (rotated 90 degrees to the right)
                // For correct Stanley controller, positive cross-track error should mean robot is to the right of path
                double normal_x = path_dy / path_length;  // Removed negative sign
                double normal_y = -path_dx / path_length; // Added negative sign
                ...
                // Cross track error is projection onto normal (positive = right of path, negative = left of path)
