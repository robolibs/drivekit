        ...
        class PIDFollower : public Controller {
          public:
            ...
            inline VelocityCommand compute_control(const RobotState &current_state, const Goal &goal,
                ...
                double min_target_distance = 2.0 * min_turning_radius_;
                if (distance_error < min_target_distance) {
                    cmd.valid = true;
                ...
                if (std::abs(heading_error) > heading_deadband) {
                    ...
                    angular_control = (config_.kp_angular * 3.0) * heading_error +
                                      config_.ki_angular * angular_integral_ + config_.kd_angular * angular_derivative;
                } else {
