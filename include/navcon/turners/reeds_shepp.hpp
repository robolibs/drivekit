...
public:
    ...
    public:
        ReedsSheppPath(const ReedsSheppPathSegmentType* type=reedsSheppPathType[0],
            ...
            double w=0., double x=0.) : type_(type), totalLength_(fabs(t) + fabs(u) + fabs(v) + fabs(w) + fabs(x)) {
    ...
    void sample(double q0[3], double q1[3], double step_size, double &length, std::vector<std::vector<double> > &points) const;
...
public:
    ReedsShepp(double min_turning_radius) : Dubins(min_turning_radius), rs_(min_turning_radius) {}
                ...
                for (int i = 0; i < 5; ++i) {
                    ...
                    switch (rs_path.type_[i]) {
                        case ReedsSheppStateSpace::RS_LEFT:
                            segment.type = forward ? ReedsSheppSegmentType::LEFT_FORWARD : ReedsSheppSegmentType::LEFT_BACKWARD;
                            break;
                        case ReedsSheppStateSpace::RS_RIGHT:
                            segment.type = forward ? ReedsSheppSegmentType::RIGHT_FORWARD : ReedsSheppSegmentType::RIGHT_BACKWARD;
                            break;
                        case ReedsSheppStateSpace::RS_STRAIGHT:
                            segment.type = forward ? ReedsSheppSegmentType::STRAIGHT_FORWARD : ReedsSheppSegmentType::STRAIGHT_BACKWARD;
                            break;
...
private:
    ...
    void sampleSpecificPath(const ReedsSheppStateSpace::ReedsSheppPath& rs_path, double q0[3], double step_size, std::vector<Pose>& waypoints) const {
        waypoints.clear();
        ...
        for (double seg = 0.0; seg <= length; seg += step_size) {
            double qnew[3] = {};
            rs_.interpolate(q0, const_cast<ReedsSheppStateSpace::ReedsSheppPath&>(rs_path), seg / radius_, qnew);
...
inline std::vector<ReedsSheppStateSpace::ReedsSheppPathSegmentType> ReedsSheppStateSpace::type(double q0[3], double q1[3]) const {
...
inline void ReedsSheppStateSpace::sample(double q0[3], double q1[3], double step_size, double &length, std::vector<std::vector<double> > &points) const {
    ReedsSheppPath path = reedsShepp(q0, q1);
...
inline std::vector<ReedsSheppStateSpace::ReedsSheppPath> ReedsSheppStateSpace::getAllReedsSheppPaths(double q0[3], double q1[3]) const {
    std::vector<ReedsSheppPath> paths;
...
inline void ReedsSheppStateSpace::interpolate(double q0[3], ReedsSheppPath &path, double seg, double s[3]) const {
    if (seg < 0.0) seg = 0.0;
