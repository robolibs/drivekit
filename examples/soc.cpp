...
int main() {
    ...
    soc_config.steering_noise = 0.5;
    soc_config.acceleration_noise = 0.3;
    soc_config.initial_steer_variance = 0.25;
    soc_config.min_steer_variance = 1e-4;
    soc_config.max_steer_variance = 1.0;
    soc_config.weight_cte = 100.0;
    soc_config.weight_epsi = 100.0;
    soc_config.weight_vel = 1.0;
    soc_config.weight_steering = 10.0;
    soc_config.weight_acceleration = 5.0;
    soc_config.ref_velocity = 2.0;
    ...
    static_obstacles.push_back({1, 10.0, 0.0, 0.8});
    static_obstacles.push_back({2, 20.0, 0.5, 0.8});
    static_obstacles.push_back({3, 30.0, -0.5, 0.8});
    ...
    for (; iteration < max_iterations; ++iteration) {
        ...
        // Visualize static obstacles (RED)
        ...
        // Visualize dynamic obstacles (GREEN)
        ...
        // Compute control
        auto cmd = soc_controller->compute_control(robot_state, goal, constraints, dt, &world_constraints);
