...
int main() {
    auto rec = std::make_shared<rerun::RecordingStream>("navcon_mppi_demo", "mppi");
    ...
    navcon::Tracker navigator(navcon::TrackerType::MPPI);

    // Configure MPPI parameters
    auto mppi_controller = dynamic_cast<navcon::pred::MPPIFollower *>(navigator.get_controller());
    if (mppi_controller) {
    ...
    for (; iteration < 2000 && !navigator.is_path_completed(); ++iteration) {
        ...
        if (cmd.valid) {
            ...
            // Visualize
            navcon::visualize::show_robot_state(rec, robot_state, "robot_mppi",
