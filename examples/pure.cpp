...
int main() {
    auto rec = std::make_shared<rerun::RecordingStream>("drivekit_pure_pursuit_demo", "pure_pursuit");
    ...
    for (int i = 0; i < 2000 && !navigator.is_path_completed(); ++i) {
        ...
        if (cmd.valid) {
            ...
            drivekit::visualize::show_robot_state(rec, robot_state, "robot_pure_pursuit", rerun::Color(0, 255, 0));
            navigator.tock();
