...
namespace waypoint {
    ...
    void Tracker::set_path(const PathGoal &path) {
        ...
        // Convert PathGoal to waypoint::Path and set it in the controller
        Path waypoint_path;
        for (const auto &waypoint : path.waypoints) {
            Pose wp;
            wp.point = waypoint;
            wp.angle = concord::Euler{0.0f, 0.0f, 0.0f}; // No specific heading required
            waypoint_path.waypoints.push_back(wp);
        }
        waypoint_path.is_closed = path.loop; // Set loop behavior

        if (controller) {
            controller->set_path(waypoint_path);
        }
    }
    ...
    void Tracker::smoothen(float interval_cm) {
        ...
        waypoint_path.is_closed = current_path->loop;
