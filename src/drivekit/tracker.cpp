...
namespace drivekit {
    ...
    VelocityCommand Tracker::tick(const RobotState &current_state, float dt,
        ...
        // once the final drivekit has been reached.
    ...
    float Tracker::get_distance_to_current_drivekit() const {
        // This method needs to be called with a robot state parameter
        // For now, return a placeholder value
        return std::numeric_limits<float>::infinity();
    }
    ...
    void Tracker::smoothen(float interval_cm) {
        ...
        for (size_t i = 0; i < current_path->drivekits.size() - 1; ++i) {
            ...
            // Calculate distance between drivekits
            ...
            // Add the end drivekit
            smoothed_drivekits.push_back(end);
        }

        // Update the current path with smoothed drivekits
        current_path->drivekits = smoothed_drivekits;
        current_drivekit_index = 0; // Reset to start
    ...
    void Tracker::tock() const {
        ...
        if (current_path.has_value() && !current_path->drivekits.empty()) {
            // Convert drivekits to 3D coordinates for visualization
            ...
            // Visualize individual drivekits as green spheres
            ...
            if (!drivekit_positions.empty()) {
                rec->log_static(entity_prefix + "/drivekits", rerun::Points3D(drivekit_positions)
                                                                  .with_colors({{0, 255, 0}}) // Green drivekits
                                                                  .with_radii({{0.07f}}));
            }

            // Highlight current target drivekit in yellow
        ...
        // Draw line from robot to current target drivekit
    ...
    void Tracker::update_drivekit_progress(const RobotState &current_state) {
        if (!current_path.has_value() || current_drivekit_index >= current_path->drivekits.size()) {
            return;
        }
        ...
        if (distance <= current_path->tolerance) {
            ...
            // DEBUG:             std::cout << ", now " << current_drivekit_index << std::endl;

            // Check if we've completed the path
            if (current_drivekit_index >= current_path->drivekits.size()) {
                if (current_path->loop && !current_path->drivekits.empty()) {
                    current_drivekit_index = 0; // Loop back to start
                } else {
                    path_completed = true;
                }
            }
