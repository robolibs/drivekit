...
namespace drivekit {
    namespace path {
        ...
        VelocityCommand StanleyFollower::compute_control(const RobotState &current_state, const Goal &goal,
            ...
            double velocity = std::max(std::abs(cmd.linear_velocity), 0.1); // Minimum velocity to avoid division issues
        ...
        void StanleyFollower::update_path_index(const Pose &current_pose) {
            ...
            // Advance path index if we're close to current drivekit or have passed it
            while (path_index_ < path_.drivekits.size() - 1) {
                ...
                // Move to next drivekit if close enough
                if (dist < 1.0) {
                    path_index_++;
                } else {
                    break;
